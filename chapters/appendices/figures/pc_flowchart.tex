...
\node (predict) [step1] {Predict $\tilde{\rho}(t_{m + 1})$};
\node (mot) [step2, right of=predict, xshift=1cm] {Calculate $\tilde{\vb{E}}(t_{m + 1})$ \\ with MoT system};
\node (evaluate) [step3, below of=mot] {Use Liouville eqn.\ \\ to produce $\partial_t \tilde{\rho}(t_{m + 1})$};
\node (correct) [step4, below of=predict] {Correct $\tilde{\rho}(t_{m + 1})$ \\ with derivative};

\draw [arrow] (predict) -- (mot) node [midway, above, gray!40] (n1) {$\tilde{\rho} \rightarrow \tilde{\vb{P}}$};
\draw [arrow] (mot) -- (evaluate);
\draw [arrow] (evaluate) -- (correct);
\draw [arrow] (correct) -- (predict) node [midway, sloped, above, gray!40] (n2) {$m + 1 \rightarrow m$};
\draw [gray!40, dashed, arrow] (correct) -- (mot) node [midway, above, sloped] (n3) {Until converged};
