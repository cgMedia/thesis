...
\begin{enumerate}
  \item At timestep $m$, use \cref{eq:predictor,eq:polarization definition} to predict $\tilde{\mathcal{A}}^{(m + 1)}$.
  ...
  \item Correct $\tilde{\mathcal{A}}^{(m + 1)}$ with \cref{eq:corrector} and $\partial_t \tilde{\mathcal{A}}^{(m + 1)}$ found in step 3.
