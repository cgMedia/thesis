...
We again consider the evolution of a collecltion of pointlike \qds{} in response to a time-varying electric field.
As in \cref{ch:quantum dots}, the $j^\text{th}$ density matrix envelope, $\tilde{\rho}_j$, evolves according to the Liouville equation
\begin{equation}
  \dv{\tilde{\rho}_j}{t} = \frac{-i}{\hbar} \commutator{\tilde{\mathcal{H}}_j\qty\big(\tilde{\vb{E}}(\vb{r}_j, t))}{\tilde{\rho}_j} - \hat{\mathcal{D}}_j \qty[\tilde{\rho_j}]
\end{equation}
and we use the same predictor/corrector integration scheme as before.
The coherent (off-diagonal) terms of $\tilde{\rho}_j$ 
This evolution describes a localized point polarization $\tilde{\vb{P}}_j(t)\delta(\vb{r} - \vb{r}_j)$ and we may write the total electric field at any point in space as
\begin{equation}
  \tilde{\vb{E}}(\vb{r}, t) = \tilde{\vb{E}}_\text{inc}(\vb{r}, t) + \tilde{\mathfrak{F}}\qty{\tilde{\vb{P}}(\vb{r}, t)}
\end{equation}
where $\tilde{\vb{E}}(\vb{r}, t)$ describes an incident laser pulse,
